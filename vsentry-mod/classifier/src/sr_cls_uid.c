		...
		if (!ent) {		
			...
			if (!ent) {
				CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
					"reason=failed to add uid for rule %u, memory allocation fail",
					rulenum);
				return SR_ERROR;
			} else {
				ent->ent_type = UID;
...
int sr_cls_uid_del_rule(enum sr_rule_type type, SR_32 uid, SR_U32 rulenum)
{
	if (uid != UID_ANY) {
		...
		if (!ent) {
			CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
				"reason=cannot del rule %u on uid %u - rule not found",
				rulenum, uid);
			return SR_ERROR;
		}
