...
{
	sk_process_info_t sk_process_info;
	sk_process_item_t *process_info_p;

	sk_process_info.pid = 4455;	
	sk_process_info.uid = 100;	
	CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
			"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXxx Arik in sr_cls_sl_process_hash_ut!!!!!!!!!!!!!!!!!!\n");
	if (sr_cls_sk_process_hash_update((void *)0xFF112233, &sk_process_info) != SR_SUCCESS) {
		CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
				"Failed to insert 0xFF112233");
	}
	CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
		"Expected ----------sk:%p pid:%d uid:%u", (void *)0xFF112233, 4455, 100);
	sr_cls_sk_process_hash_print();

	if (!(process_info_p = sr_cls_sk_process_hash_get((void *)0xFF112233))) {
		CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH, "FAILE reading 0xFF112233");
		return;
	}
	CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
		"Expected get ---------- pid:%d uid:%d <<<<>>> %d %d", 4455, 100, process_info_p->process_info.pid, process_info_p->process_info.uid);

	sk_process_info.pid = 4477;	
	sk_process_info.uid = 500;	
	if (sr_cls_sk_process_hash_update((void *)0xFF112244, &sk_process_info) != SR_SUCCESS) {
		CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
				"Failed to insert 0xFF112233");
	}
	CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
		"Expected ----------sk:%p pid:%d uid:%u \n          sk:%p pid:%d uid:%u",
		(void *)0xFF112233, 4455, 100, (void *)0xFF112244, 4477, 500);
	sr_cls_sk_process_hash_print();

	sk_process_info.pid = 3000;	
	sk_process_info.uid = 222;	
	if (sr_cls_sk_process_hash_update((void *)0xFF112233, &sk_process_info) != SR_SUCCESS) {
		CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
				"Failed to insert 0xFF112233");
	}
	CEF_log_event(SR_CEF_CID_SYSTEM, "error", SEVERITY_HIGH,
		"Expected ----------sk:%p pid:%d uid:%u \n          sk:%p pid:%d uid:%u",
		(void *)0xFF112233, 3000, 222, (void *)0xFF112244, 4477, 500);
	sr_cls_sk_process_hash_print();
