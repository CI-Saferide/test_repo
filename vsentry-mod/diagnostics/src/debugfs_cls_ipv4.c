...
struct debugfs_buf_list_node_t
{
	struct debugfs_buf_list_node_t *next;
	size_t len;
	unsigned char *buf;
};
...
static struct debugfs_network_ent_t debugfs_network[SR_MAX_RULES];
static SR_U16 store_table_rule_num;
static struct debugfs_buf_list_node_t *buf_list_head;
				...
				if (sr_cls_port_table[j]->buckets[i].head != NULL){
					...
					while (curr != NULL){
						if(sal_test_bit_array(rule,&curr->rules)){
							...
							sal_sprintf(debugfs_network[rule].proto,"%s",((j == 0) || (j == 2)) ? "TCP":"UDP");
...
static size_t debugfs_write_ipv4_table_title(char __user *user_buf, size_t count, loff_t *ppos, size_t *used_count)
{
...
static size_t store_table(char __user *user_buf, size_t count, loff_t *ppos, SR_U8 first_call)
{
	...
	if (first_call) {
		rt = debugfs_write_ipv4_table_title(user_buf, count, ppos, &used_count); // title
		if (rt)
			return rt;
	...
	for (; i < SR_MAX_RULES; i++) {
		if (debugfs_network[i].rule) {
			
			len = sal_sprintf(buf,"%d\t%015s/%d\t%015s/%d\t%d\t%d\t%s\t%s\t%s\t\t%s\n",
				debugfs_network[i].rule,
				debugfs_network[i].src_ipv4,
				debugfs_network[i].src_netmask_len,
				debugfs_network[i].dst_ipv4,
				debugfs_network[i].dst_netmask_len,
				debugfs_network[i].s_port,
				debugfs_network[i].d_port,
				debugfs_network[i].proto,
				debugfs_network[i].uid_buff,
				debugfs_network[i].inode_buff,
				debugfs_network[i].actionstring);
...
static size_t store_rule(SR_16 rule_find, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	rt = debugfs_write_ipv4_table_title(user_buf, count, ppos, &used_count);
	if (rt)
		return rt;

	if (debugfs_network[rule_find].rule == rule_find) {

		len = sal_sprintf(buf,"%d\t%015s/%d\t%015s/%d\t%d\t%d\t%s\t%s\t%s\t\t%s\n",
				debugfs_network[rule_find].rule,
				debugfs_network[rule_find].src_ipv4,
				debugfs_network[rule_find].src_netmask_len,
				debugfs_network[rule_find].dst_ipv4,
				debugfs_network[rule_find].dst_netmask_len,
				debugfs_network[rule_find].s_port,
				debugfs_network[rule_find].d_port,
				debugfs_network[rule_find].proto,
				debugfs_network[rule_find].uid_buff,
				debugfs_network[rule_find].inode_buff,
				debugfs_network[rule_find].actionstring);
...
static size_t rn_printnode(struct radix_node *n, SR_U32 level, struct debugfs_buf_list_node_t **pknode,
		size_t count, size_t *used_count)
{
	size_t rt = 0;
	bit_array matched_rules;
	SR_16 rule;
	struct debugfs_buf_list_node_t *new_node;
	char *kbuf;
	...
	if (n != NULL && pknode != NULL && *pknode != NULL) {
		...
		if (*used_count + SR_MAX_PATH > count) {
			...
			new_node = SR_ZALLOC(sizeof(struct debugfs_buf_list_node_t));
			if (!new_node) {
...
static size_t rn_printtree(struct radix_head *h, char __user *user_buf, size_t count, loff_t *ppos, SR_U8 first_call)
{
	...
	struct debugfs_buf_list_node_t *temp;
	...
	if (first_call) {
		...
		buf_list_head = SR_ZALLOC(sizeof(struct debugfs_buf_list_node_t));
		if (!buf_list_head) {
			pr_warn("%s failed to allocate memory\n",__func__);
			return -ENOMEM;
		}
...
int walktree_debugfs_print_rule(struct radix_node *node, void *data)
{
	...
	while ((rule = sal_ffs_and_clear_array (&matched_rules)) != -1) {
		...
		if (*dir == SR_DIR_SRC) {
			...
			if (debugfs_network[rule].rule == rule && debugfs_network[rule].src_flag == 1) {
				continue;
			}
			debugfs_network[rule].rule = rule;
			debugfs_network[rule].src_flag = 1;
			debugfs_network[rule].src_netmask_len = abs(node->rn_bit + 33);
			...
			if (debugfs_network[rule].rule == rule && debugfs_network[rule].dst_flag == 1) {
				continue;
			}
			debugfs_network[rule].rule = rule;
			debugfs_network[rule].dst_flag = 1;
		...
		debugfs_network[rule].uid = get_uid_for_rule(sr_cls_uid_table,rule,UID_HASH_TABLE_SIZE,SR_NET_RULES);
		if (debugfs_network[rule].uid != -1) {
			if (debugfs_network[rule].uid == 0) {
				sal_sprintf(debugfs_network[rule].uid_buff, "%s", "ANY");
			} else
				sal_sprintf(debugfs_network[rule].uid_buff, "%d", debugfs_network[rule].uid);
		} else {
			sal_sprintf(debugfs_network[rule].uid_buff, "%s", "N/A");
		}

		// putting work for the BIN
		debugfs_network[rule].inode = get_exec_for_rule(sr_cls_exec_file_table,rule,EXEC_FILE_HASH_TABLE_SIZE,SR_NET_RULES);
		if (debugfs_network[rule].inode != -1) {
			if (debugfs_network[rule].inode == 0) {
				sal_sprintf(debugfs_network[rule].inode_buff, "%s", "ANY");
			} else
				sal_sprintf(debugfs_network[rule].inode_buff, "%u", debugfs_network[rule].inode);
		}

		debugfs_network[rule].action = sr_db->sr_rules_db[SR_NET_RULES][rule].actions;
		if (debugfs_network[rule].action & SR_CLS_ACTION_DROP) {
			sal_sprintf(debugfs_network[rule].actionstring, "Drop");
		} else if (debugfs_network[rule].action & SR_CLS_ACTION_ALLOW) {
			sal_sprintf(debugfs_network[rule].actionstring, "Allow");
		}
		if (debugfs_network[rule].action & SR_CLS_ACTION_LOG) {
			if (strlen(debugfs_network[rule].actionstring) == 0) {
				sal_sprintf(debugfs_network[rule].actionstring, "Log");
			} else {
				strcat(debugfs_network[rule].actionstring, "_log");
			}
		}
	}
	return 0;
...
static void fetch_cls_ipv4(void)
{
	...
	sal_memset(debugfs_network, 0, sizeof(debugfs_network));
	...
	dir = SR_DIR_SRC;
	rn_walktree(sr_cls_ipv4_table[SR_DIR_SRC], walktree_debugfs_print_rule, &dir);
	dir = SR_DIR_DST;
	rn_walktree(sr_cls_ipv4_table[SR_DIR_DST], walktree_debugfs_print_rule, &dir);
}
