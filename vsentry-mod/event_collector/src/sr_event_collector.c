...
static inline SR_BOOL sr_ec_sample_period_exceeded(SR_U8 type)
		...
		case  MOD2ENG_BUF:
			switch (event_type) {
				...
				case SR_EVENT_PROCESS_DIED:
					sr_ec_append_event(buf_type, event_type, data, sizeof(struct sr_ec_process_died_t), SR_FALSE);
					break;
#endif
				default:
					break;
...
void sr_ec_append_event(SR_U8 buf_type, SR_U8 event_type, void *sample_data, SR_U32 data_length, SR_BOOL is_imidiate) 
{
	...
	if ( sr_ec_offset[buf_type] && ((sr_ec_offset[buf_type]+data_length > size) || // buffer full
			(sr_ec_sample_period_exceeded(buf_type))) ) { // time based constraint
